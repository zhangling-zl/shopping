  ...
  <div>
    <div class="box">
      ...
      <van-goods-action>
        ...
        <van-goods-action-icon icon="cart-o" text="购物车" :badge="shopListAA.length" />
...
export default {
        ...
        this.$api
          ...
          .then((res) => {
            if (res.code === 200) {
              ...
              this.shopList++
              localStorage.setItem("shopListAA",res.shopList)
            }
          })
          .catch((err) => {
            console.log(err);
  ...
  mounted() {
    ...
    //     console.log(err);
    //   });
    this.shopListAA= JSON.parse(localStorage.getItem('shopListAA'))
  },
  watch: {},
  computed: {
    //  str(){
