  ...
  <div>
    <div class="box">
      ...
      <van-popup
        ...
        :style="{ height: '40%' }"
      >
        <div class="flex a-c fomm">
          <img :src="goodsOne.image" class="P-img mr10" />
          <div>
            <div class="P-text1 f18 mb15">{{goodsOne.name}}scgyhgx5t4reg46bnht64</div>
            <div class="P-text2 f600">￥{{goodsOne.orl_price | toFixed}}</div>
          </div>
        </div>
        <div class="fomm flex">
          <div>
            ...
            <div class="flex a-c mt10">
              <div class="f14 mr10" style="color: rgb(166,166,195);">剩余件数 {{goodsOne.amount}} 件</div>
              <div class="f14" style="color: rgb(255, 90, 94);">每人限购50件</div>
            </div>
          </div>
          <div class="flex a-c ml30">
            <div class="dv1" @click="minus">-</div>
            <div class="dv3">{{count}}</div>
            <div class="dv2" @click="addgood">+</div>
          </div>
        </div>
        <div>
          <van-button type="danger" class="button" @click="Buynow">立即购买</van-button>
        </div>
      </van-popup>
    </div>
  </div>
  ...
  methods: {
    ...
    Buynow() {
      if (this.user) {
        let arr = [];
        arr.push({
          mallPrice: this.goodsOne.orl_price, //价格
          name: this.goodsOne.name, //商品名称
          count: this.count, //商品数量
          image_path: this.goodsOne.image, //商品图片
          idDirect: true, //用来判断是购物车结算还是直接购买(直接购买为true,购物车结算为false)
          cid: this.goodsOne.id, //单个商品id
        });
        this.$router.push("/settlement"); //跳转到结算页面
        sessionStorage.setItem("arrAll", JSON.stringify(arr)); //存在sessionStorage（浏览器里面）
        // console.log(arr,'单个商品的数据');
      } else {
        Dialog.confirm({
          title: "系统检测到当前状态未登录",
          message: "是否立即前往登录/注册页面",
