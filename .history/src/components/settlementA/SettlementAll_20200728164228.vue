<template>
  <div>
    ...
    <!--<div class="wrapper" ref="wrapper">
      <ul class="content">
        <li class="p10" v-if="shopListAll.length>0">
...
export default {
  ...
  methods: {
    ...
    onSubmit() {
       this.$api
        .postOrder({
          address: this.defaultAdd, //收货地址
          tel: this.defaultAdd.tel, //电话
          orderId: this.arr, //所有商品的id(数组)
          totalPrice: this.Totalprice, //总价格
          idDirect: false, //用来判断是购物车结算还是直接购买(直接购买为true,购物车结算为false)
          count: this.count, //数量
        })
        .then((res) => {
          console.log(res);
  ...
  mounted() {
    ...
    this.shopListAll = JSON.parse(sessionStorage.getItem("arr"));
    console.log(this.shopListAll);
  },
  watch: {},
  computed: {
    //总价格（计算属性）
