<template>
  <div>
    <div class="address flex j-c a-c" v-if="addressAll.length=0">
      <div @click="address">点击添加收货地址</div>
    </div>
    <div class="address">
      <div class="flex a-c j-b">
        <div class="text1 f20">收货人{{defaultAdd.name}}</div>
        <div class="text2 f20">{{defaultAdd.tel}}</div>
      </div>
      <div class="text3">收货地址:{{defaultAdd.address}}</div>
      <div>收货不便时，可选择代收货服务</div>
    </div>
