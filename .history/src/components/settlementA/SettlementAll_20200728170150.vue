<template>
  <div>
    ...
    <img src="../../assets/img/caitiao.jpg" class="img2" />
        <div class="p10" v-if="arrAll.length>0">
          <div v-for="(item,index) in arrAll" :key="index">
            <div class="from">
    ...
    <van-submit-bar :price="item.totalPrice" button-text="提交订单" @submit="onSubmit" />
  </div>
</template>
<script>
...
export default {
      ...
      this.arrAll.map((item)=>{
       ...
       this.$api
        .postOrder({
          ...
          idDirect: this.arrAll[0].idDirect, //用来判断是购物车结算还是直接购买(直接购买为true,购物车结算为false)
          count: this.arrAll[0].count, //商品数量
        })
        .then((res) => {
          console.log(res);
        })
  ...
  mounted() {
    ...
    this.arrAll = JSON.parse(sessionStorage.getItem("arrAll"));
