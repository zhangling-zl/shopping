    ...
    Save() {
      ...
      this.$api
        .postSaveUser({
          ...
          year: this.user.userInfo.year,
          month: this.user.userInfo.month,
          day: this.user.userInfo.day,
          id: this.user.userInfo.username,
      ...
      .then((res) => {
        ...
        if (user !== null) {
          (this.username = user.userInfo.username),
            ...
            (this.date = `${user.userInfo.year}年${user.userInfo.month}月${user.userInfo.day}日`);
        }
      })
      .catch((err) => {
        console.log(err);
