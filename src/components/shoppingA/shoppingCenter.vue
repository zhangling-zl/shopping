        ...
        <li class="p10" v-if="shopListAll.length">
          ...
          <div class="flex">
            ...
            >
              ...
              <div>
                <van-button color="rgb(230,5,127)" size="small" @click="Settlement">去结算</van-button>
              </div>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    //点击跳转到结算页面
    Settlement() {
      let arr = [];//定义一个空数组,装选中的数据，item（每一项）
      this.shopListAll.map((item) => {
        if (item.check === true) {
          arr.push(item)
        }
      });
      //循环数组 ，给他的每一项增加一个idDirect为false
      arr.map((item)=>{
        item['idDirect'] = false
      })
      //跳转到结算页面
      this.$router.push("/settlement");
      sessionStorage.setItem('arrAll',JSON.stringify(arr))
    },
  },
  mounted() {
    //接收从首页和详情页传递过来的id值
